      ...
      <S.WrapperContents>
        ...
        {props.adoptions.map((adoption) => (
          <S.CommunityBlock
            key={adoption.id}
            onClick={() =>
              props.handleRequireModal(`/community/${adoption.id}`)
            }
          >
            <S.CommunityImg>
              <Image
            ...
            <S.CommunityInfoBlock>
              <S.CommunityTitle>
                {props.truncateString(adoption.title, 15)}
              </S.CommunityTitle>
              <S.CommunityText>
                {props.truncateString(adoption.content, 26)}
              </S.CommunityText>
              <S.CommunityName>{adoption.name}</S.CommunityName>
        ...
        <S.CommunityAddIcon
          onClick={() =>
            props.handleRequireModal("/community/write?type=adoption")
          }
        >
          <Image
        ...
        {props.adoptions.length > 0 && (
          <S.MoreButton onClick={props.loadAdoptionList}>더 보기</S.MoreButton>
        )}
      </S.WrapperContents>
    </>
  );
