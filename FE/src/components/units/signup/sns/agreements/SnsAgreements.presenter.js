import * as S from "./SnsAgreements.styles";
import SignupHeader from "../../SignupHeader/SignupHeader.container";
import Progress from "../../component/Progress";
import Image from "next/image";

export default function SnsAgreementsUI(props) {
  return (
    <>
      <S.WrapperContainer>
        <SignupHeader />
        <S.ProgressBarBlock>
          <S.ProgressBar
            value={Progress({ startValue: 0, max: 33, interval: 10 })}
            max={33}
          />
        </S.ProgressBarBlock>
        <S.InfoContainer>
          ...
          <S.NextButtonItem
            type="submit"
            onClick={() => props.handledNextButton("/login/signup/sns/01")}
          >
            다음
          </S.NextButtonItem>
        </S.InfoContainer>
