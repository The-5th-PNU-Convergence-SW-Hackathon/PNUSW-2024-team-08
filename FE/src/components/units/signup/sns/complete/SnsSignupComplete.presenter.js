import * as S from "./SnsSignupComplete.styles";
import Image from "next/image";

export default function SnsSignupCompleteUI(props) {
  return (
    <>
      <S.WrapperContainer>
        ...
        <S.ComplateImageContainer>
          ...
          <S.BlurrBlock>
            <S.NextButtonBlock>
              <S.NextButtonItem
                onClick={() => {
                  if (props.authProvider === "KAKAO") {
                    props.handleKakaoLoginClick();
                  } else {
                    props.handleGoogleLoginClick();
                  }
                }}
              >
                지금 시작하기
